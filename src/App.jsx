...
function AssetsTab({assets}){
  ...
  const [holdings, setHoldings] = useLocalState("holdings", [
    { id:"BTC", name:"비트코인", avg: 100_000_000, qty: 0.3 }, // 1억 평단, 3천만원 넣은 기본값
  ]);
...
export default function App() {
  ...
  const [query, setQuery] = useState(""); const [sort, setSort] = useState("popular"); const [tab, setTab] = useState("market"); const [live, setLive] = useState(true);
  const handleLivePrice = (id, price) => { setAssets((old) => old.map((a) => { if (a.id !== id) return a; const prev = a.price; const point = { i: (a.data[a.data.length - 1]?.i ?? 0) + 1, v: price }; const data = [...a.data.slice(-39), point]; const first = data[0].v; const change = ((price - first) / first) * 100; return { ...a, price, data, change, lastPrice: prev }; })); if (id === 'BTC') try { localStorage.setItem('btc_last_live', String(price)); } catch {} };
